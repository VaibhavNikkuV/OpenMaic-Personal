...
- Do not assume the OpenClaw agent's own model or API key will be reused by Drishti AI Tutor.
- Drishti AI Tutor classroom generation uses Drishti server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Drishti AI Tutor server-side config files may control provider selection and defaults.
  ...
  "skills": {
    "entries": {
      "drishti": {
        ...
        "config": {
          "accessCode": "sk-xxx",
          "repoDir": "/path/to/drishti-ai-tutor",
...
If no `accessCode` in config, ask the user how they want to use Drishti AI Tutor:

1. **Use hosted Drishti AI Tutor** (recommended for quick start) — Requires an access code. No local setup needed.
...
Use this when the user has not installed Drishti AI Tutor yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Drishti AI Tutor

After the user has chosen a startup mode and configured keys, start Drishti AI Tutor using the chosen method, then verify the service with `GET {url}/api/health`.
