...
- Drishti AI Tutor is healthy at the selected `url`
- Provider keys are configured

> **Hosted mode**: If using a hosted Drishti AI Tutor deployment, all
...
- Do not try to recover from auth, provider, model, or base URL errors by changing request parameters. Tell the user to fix Drishti AI Tutor server-side config and retry only after they confirm.
